...
| | |
|---|---|
| Hợp đồng đầu ra | `ScanResult{image, verdict, reasons[], metrics}`; **29 mã lý do**, mã nào cũng có `hint` + `audience`; bất biến `pass ⟺ reasons == []` ép ở mức code |
| Bộ đo | **401 test** + CI (lint · test 3.9/3.12 · build wheel). Bài quan trọng nhất: *không false pass* trên 9 ảnh hỏng dựng bằng OpenCV |
